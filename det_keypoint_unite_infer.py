...
def topdown_unite_predict_video(detector,
    ...
    while (1):
        ...
        '''
        store_res.append([
            np.array(index), np.array(keypoint_res['bbox']),
            [keypoint_res['keypoint'][0], keypoint_res['keypoint'][1]]
        ])
        ...
        if camera_id != -1:
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
    writer.release()
    store_res = np.array(store_res, dtype=object)
    np.save('output/'+FLAGS.video_file.split('/')[-1].split('.')[0]+'.npy', store_res)  # 保存关键点坐标
    if save_res:
        """
...
if __name__ == '__main__':
    ...
    FLAGS.keypoint_threshold = 0.1
    FLAGS.det_threshold = 0.1
